(() => {
  ...
  const loginUrl = new URL('./auth.html', location.href).href;
  let state = 'signin';
  let pendingEmail = '';

  function status(text, ok = false) {
    const el = $('status');
    if (el) { el.textContent = text; el.className = 'status ' + (ok ? 'ok' : ''); }
  }
  function render(next) {
    ...
    $('links').innerHTML = (reset || verify || recovery)
      ...
      : signup
        ? '<button type="button" onclick="mode(\'signin\')">Already have an account?</button>'
        : '<button type="button" onclick="mode(\'signup\')">Create account</button><button type="button" onclick="mode(\'reset\')">Forgot password?</button>';
    if (verify) {
  ...
  async function submit(e) {
    ...
    try {
      if (state === 'signin') {
        ...
        status('Signed in. Opening Living India…', true);
        location.href = home;
        return;
      }
      if (state === 'signup') {
