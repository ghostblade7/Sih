/* Living India auth hard-fix v2 — block only the legacy React demo login. */
(function(){
  ...
  function kill(){
    document.querySelectorAll('.modal-overlay,.modal,[role="dialog"]').forEach(el=>{
      if(!isAuthRoot(el)&&!el.closest?.('#li-auth-root')&&LEGACY.test((el.textContent||'').replace(/\s+/g,' '))){el.remove();}
    });
  }
  function open(){
    kill();
  ...
  const start=()=>{kill();new MutationObserver(kill).observe(document.body,{childList:true,subtree:true});setInterval(kill,250);};
