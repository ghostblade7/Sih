...
.li-real-map svg{display:block;width:100%;height:620px;touch-action:pan-x pan-y;cursor:grab}.li-real-map svg:active{cursor:grabbing}
.li-map-toolbar{position:absolute;right:16px;top:16px;display:flex;gap:7px;z-index:20;background:rgba(248,239,218,.96);border:1px solid rgba(101,62,37,.25);border-radius:12px;padding:6px;box-shadow:0 8px 22px rgba(63,37,21,.12)}
.li-map-toolbar button{width:42px;height:42px;border:1px solid rgba(101,62,37,.2);border-radius:9px;background:#f9f0df;color:#50301f;font-size:21px;font-weight:700;cursor:pointer;pointer-events:auto}
.li-map-toolbar button:hover{background:#ead5ad}
.li-map-title{position:absolute;left:18px;top:16px;z-index:20;padding:10px 14px;border-radius:12px;background:rgba(248,239,218,.94);border:1px solid rgba(101,62,37,.2);color:#50301f;pointer-events:none}.li-map-title b{display:block;font-size:15px;letter-spacing:.08em;text-transform:uppercase}.li-map-title small{display:block;margin-top:2px;opacity:.7;font-size:11px}
...
function makeMap(host,d3){
  ...
  fetch(INDIA_GEOJSON,{cache:"force-cache"}).then(r=>{if(!r.ok)throw new Error("Map data failed");return r.json()}).then(data=>{
    ...
    root.selectAll("path").data(features).join("path")
      .attr("class","li-state")
      .attr("d",path)
      .attr("tabindex",0)
      ...
      .on("click",function(event,d){
        ...
        const handler=window.__livingIndiaStateHandler;
        if(typeof handler==="function") handler(stateName(d));
      })
    ...
    root.selectAll("text").data(features).join("text")
    ...
    // Prevent D3's click suppression from swallowing taps on a state.
    svg.on("click.mapState",function(event){
      const target=event.target;
      if(target && target.classList && target.classList.contains("li-state")) return;
    });

    toolbar.querySelector('[data-zoom="in"]').onclick=e=>{e.stopPropagation();svg.transition().duration(180).call(zoom.scaleBy,1.5)};
    toolbar.querySelector('[data-zoom="out"]').onclick=e=>{e.stopPropagation();svg.transition().duration(180).call(zoom.scaleBy,0.67)};
    toolbar.querySelector('[data-zoom="reset"]').onclick=e=>{e.stopPropagation();svg.transition().duration(180).call(zoom.transform,d3.zoomIdentity)};

    host.dataset.realMapReady="1";host.dataset.realMapState="ready";
  }).catch(err=>{
    console.error("Living India map data error",err);
    host.dataset.realMapState="error";
    host.innerHTML='<div style="padding:32px;text-align:center;color:#50301f;font-family:Georgia,serif"><h3>India map could not load</h3><p>Please refresh once the map data connection is available.</p><button type="button" data-map-retry style="margin-top:12px;padding:10px 16px;border-radius:9px;border:1px solid #76502f;background:#f9f0df;color:#50301f;cursor:pointer">Retry map</button></div>';
    host.querySelector('[data-map-retry]').onclick=()=>{delete host.dataset.realMapState;makeMap(host,d3)};
  });
}

async function initRealMap(host){
  if(!host||host.dataset.realMapReady||host.dataset.realMapState==="loading")return;
  injectStyle();
  try{const d3=await getD3();makeMap(host,d3)}
  catch(err){console.error("Living India map error",err);host.dataset.realMapState="error"}
}
