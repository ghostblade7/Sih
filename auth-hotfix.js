/* Living India auth trigger hotfix v3 — ignore the real Supabase auth dialog. */
(function(){
  const LEGACY_RE=/Demo Sign In|Enter Living India|Sign in to save discoveries|WELCOME BACK/i;
  function isLegacy(el){
    if(!el||el.id==='li-auth-root'||el.closest?.('#li-auth-root')) return false;
  ...
  document.addEventListener('click',function(e){
    ...
    if(isAvatar||isLogin){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();openAuth();}
  },true);
